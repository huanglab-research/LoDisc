...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch.startswith('vit'):
        ...
        linear_keyword = 'fc'
    # print(linear_keyword) # heads
    
    classifier = Classifier(768, args.class_num).cuda(args.gpu)

    # freeze all layers but the last fc
    for name, param in model.named_parameters():
        if name not in ['%s.head.weight' % linear_keyword, '%s.head.bias' % linear_keyword]:
            param.requires_grad = False
    # init the fc layer
    getattr(model, linear_keyword).head.weight.data.normal_(mean=0.0, std=0.01)
    getattr(model, linear_keyword).head.bias.data.zero_()
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            ...
            # print({"%s.weight" % linear_keyword, "%s.bias" % linear_keyword}) # {'heads.bias', 'heads.weight'}
    ...
    optimizer = torch.optim.SGD(classifier.parameters(), init_lr,
...
def sanity_check(state_dict, pretrained_weights, linear_keyword):
    ...
    for k in list(state_dict.keys()):
        ...
        if '%s.head.weight' % linear_keyword in k or '%s.head.bias' % linear_keyword in k:
            continue
