...
def main_worker(gpu, ngpus_per_node, args):
    global best_top1, best_top5, best_mAP
    args.gpu = gpu
    ...
    if args.arch.startswith('vit'):
        model = Disc.builder.MoCo_ViT(
            partial(vits.__dict__[args.arch], stop_grad_conv1=args.stop_grad_conv1),
            args.moco_dim, args.moco_mlp_dim, args.moco_t)
    else:
        model = Disc.builder.MoCo_ResNet(
    ...
    if args.optimizer == 'lars':
        optimizer = Disc.optimizer.LARS(model.parameters(), args.lr,
    ...
    augmentation1 = [
        ...
        transforms.RandomApply([Disc.loader.GaussianBlur([.1, 2.])], p=1.0),
    ...
    augmentation2 = [
        ...
        transforms.RandomGrayscale(p=0.2),
        transforms.RandomApply([Disc.loader.GaussianBlur([.1, 2.])], p=0.1),
        transforms.RandomApply([Disc.loader.Solarize()], p=0.2),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        normalize
    ]
    ...
    train_dataset = datasets.ImageFolder(
        traindir,
        Disc.loader.TwoCropsTransform(transforms.Compose(augmentation1), 
    ...
    val_loader = torch.utils.data.DataLoader(
        ...
        num_workers=args.workers, pin_memory=True) # False->True
    
    if args.evaluate:
        validate(val_loader, model, args, epoch)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        top1, top5, mAP = validate(val_loader, model, args, epoch)
...
def train(train_loader, model, optimizer, scaler, summary_writer, epoch, args):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        # compute output
        logits1, labels1, _, _ = model.module.images(images[0], images[1], moco_m)       

        # acc1/acc5 # (K+1)-way contrast classifier accuracy
...
def validate(val_loader, model, args, epoch):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            if args.gpu is not None:
                images = images.cuda(args.gpu, non_blocking=True) # [batch_size, 3, 224, 224]
                target = target.cuda(args.gpu, non_blocking=True) # [batch_size]

            # compute output
            output = model.module.inference(images) # [batch_size, hidden_dim]
            data.append(output)
            label.append(target)
